...
}).as(sql`SELECT vote_meta.vote_number, vote_meta.vote_id, vote_meta.bill_id, vote_meta.chamber, vote_meta.date, vote_meta.result, vote_meta.category, vote_meta.nomination_title, vote_meta.source_filename, legislators.name AS sponsor_name, legislators.party AS sponsor_party FROM vote_meta LEFT JOIN bills ON vote_meta.bill_id::text = bills.bill_id::text LEFT JOIN legislators ON bills.sponsor_id::text = legislators.bioguide_id::text`);